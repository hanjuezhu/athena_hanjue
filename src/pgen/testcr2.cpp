...
//======================================================================================
/*! \file Second test case in Jiang and Oh 2018
 *  \brief Triangular profile
 *
 *====================================================================================*/
...
void LinearInnerBoundaryMHD(MeshBlock *pmb, Coordinates *pco, AthenaArray<Real> &prim,
     FaceField &b, Real time, Real dt, int is, int ie, int js, int je, 
     int ks, int ke, int ngh);

void LinearOuterBoundaryMHD(MeshBlock *pmb, Coordinates *pco, AthenaArray<Real> &prim,
     FaceField &b, Real time, Real dt, int is, int ie, int js, int je, 
     int ks, int ke, int ngh);

void CRLinearInnerBoundaryCR(MeshBlock *pmb, Coordinates *pco, CosmicRay *pcr,
...
void CRLinearOuterBoundaryCR(MeshBlock *pmb, Coordinates *pco, CosmicRay *pcr,
    const AthenaArray<Real> &w, FaceField &b,
    AthenaArray<Real> &u_cr, Real time, Real dt, int is, int ie,
    int js, int je, int ks, int ke, int ngh);
...
void Mesh::InitUserMeshData(ParameterInput *pin) {
  if (mesh_bcs[BoundaryFace::inner_x1] == GetBoundaryFlag("user")) {
    EnrollUserBoundaryFunction(BoundaryFace::inner_x1, LinearInnerBoundaryMHD);
    if(CR_ENABLED){
      EnrollUserCRBoundaryFunction(BoundaryFace::inner_x1, CRLinearInnerBoundaryCR);
    }
  }
  if (mesh_bcs[BoundaryFace::outer_x1] ==  GetBoundaryFlag("user")) {
    EnrollUserBoundaryFunction(BoundaryFace::outer_x1, LinearOuterBoundaryMHD);
    if(CR_ENABLED){
      EnrollUserCRBoundaryFunction(BoundaryFace::outer_x1, CRLinearOuterBoundaryCR);
    }
  }
}
  ...
  for(int k=ks; k<=ke; ++k) {
    for (int j=js; j<=je; ++j) {
      for (int i=is; i<=ie; ++i) {
        ...
        if(CR_ENABLED){
            pcr->u_cr(CRE,k,j,i) = x1 > 0.0 ? 2.0 - x1 : 2.0 + x1;
...
// z component
        ...
        for(int i=il; i<=iu; ++i){
          ...
          Real va1 = 1.0; // Set to 1 for test
          Real va2 = bcc(IB2,k,j,i)*inv_sqrt_rho;
          Real va3 = bcc(IB3,k,j,i)*inv_sqrt_rho;

          Real va = sqrt(va1*va1 + va2*va2 + va3*va3);
    ...
    for (int j=js; j<=je; ++j) {
      for (int i=1; i<=(NGHOST); ++i) {
        for (int n=0; n<NHYDRO; ++n) {
          prim(n,k,j,is-i) = 2.*prim(n,k,j,is-i+1) - prim(n,k,j,is-i+2);
        }
      }
    }
  }
     ...
     int ks, int ke, int ngh)
{
  for (int k=ks; k<=ke; ++k) {
    for (int j=js; j<=je; ++j) {
      for (int i=1; i<=(NGHOST); ++i) {
        for (int n=0; n<NHYDRO; ++n) {
          prim(n,k,j,ie+i) = 2.*prim(n,k,j,ie+i-1) - prim(n,k,j,ie+i-2);
        }
      }
    }
  }
    ...
    int js, int je, int ks, int ke, int ngh)
{
  for (int k=ks; k<=ke; ++k) {
    for (int j=js; j<=je; ++j) {
      for (int i=1; i<=NGHOST; ++i) {
        for (int n=0; n<NCR; ++n) {
          u_cr(n,k,j,is-i) = 2.0*u_cr(n,k,j,is-i+1) - u_cr(n,k,j,is-i+2);
        }
      }
    }
  }
}// end CRLinearInnerBoundaryCR
    ...
    int js, int je, int ks, int ke, int ngh)
{
  for (int k=ks; k<=ke; ++k) {
    for (int j=js; j<=je; ++j) {
      for (int i=1; i<=NGHOST; ++i) {
        for (int n=0; n<NCR; ++n) {
          u_cr(n,k,j,ie+i) = 2.0*u_cr(n,k,j,ie+i-1) - u_cr(n,k,j,ie+i-2);
        }
      }
    }
  }
}// end CRLinearOuterBoundaryCR